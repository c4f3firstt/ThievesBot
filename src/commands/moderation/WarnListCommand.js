...
module.exports = class UnwarnCommand extends Command {
  ...
  async run(ctx) {
    ...
    const userWarns = await this.client.database.Warns.find({ userID: user.id }, null, { sort: { warnNumber: 1 } })

    if (userWarns.length < 1) return ctx.send("Este usuário não possui avisos")

    const embed = new MessageEmbed()
      .setTitle(`Avisos na Mega de: \`${user.user.tag}\`\ `)
      .setColor('#ff0000')
